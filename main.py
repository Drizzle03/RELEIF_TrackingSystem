...
class VideoTracker(object):
    ...
    def run(self):
        ...
        while self.vdo.grab():
            ...
            # post-processing ***************************************************************
            # visualize bbox  ********************************
            if len(outputs) > 0: #박스가 있는가?
                sense = True #처음 감지 O
                opos = center_point #좌표값 저장
                time_gone = 0 #사라진 시간 초기화
                locate = 'living' #위치 변경
                ...
                if sense == True: #이미 박스가 전에 검출 & 박스가 사라진 경우
                    if locate == 'living': #위치가 거실이었다면
                        time_gone +=1
                        if time_gone >= 30: #객체가 사라진지 20프레임이 지났다면
                            #좌표 계산
                            r1 = math.sqrt((room_pos[0][0]-center_point[0])**2) + math.sqrt((room_pos[0][1]-center_point[1])**2)
                            r2 = math.sqrt((room_pos[1][0]-center_point[0])**2) + math.sqrt((room_pos[1][1]-center_point[1])**2)

                            if r1<r2: #만약에 r1 좌표랑 가깝다면
                                locate = 'room1'
                            else: 
                                locate = 'room2'
            ...
            # locate 변경 내역이 있다면, 스프레드시트에 업로드**********
            if locate != update: #현위치와 업데이트 위치가 다르다면
                current_time = datetime.now()
                update = locate
                #스프레드 시트에 current_time / id_locate 업로드
                print(current_time, locate)
    ...
    def image_track(self, im0):
        ...
        if det is not None and len(det):  # det: (#obj, 6)  x1 y1 x2 y2 conf cls
            ...
            for *xyxy, _, _ in reversed(det):
                    ...
                    # print('object pos : %s ' % str(center_point))
