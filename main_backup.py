...
area = ['living room','kitchen','bathroom','room3','room2','out','room1']
...
class VideoTracker(object):
    ...
    def run(self):
        ...
        while self.vdo.grab():
            ...
            # post-processing ***************************************************************
            # visualize bbox  ********************************
            if identities>0: #id가 있는가?
                if len(outputs) > 0: #박스가 있는가?
                    if id_locate_tf != True:
                        id_locate_num +=1 #변경 내역이 있음.
                        id_locate_tf = True
                    bbox_xyxy = outputs[:, :4]
                    ...
                    if id_locate_tf:
                        id_locate_num +=1 #변경 내역이 있음.
                        id_locate_tf = False #거실 외 공간에 있음

                    #유클리드식으로 가까운 영역 좌표 계산 후 저장
                    #id_locate = area[n]
                    id_locate_tf = False
                    pass
            else:
            ...
            if memo != id_locate_num:
                memo = id_locate_num #변경된 내역이 있어서 처리 완료함.
                current_time = datetime.now()
                #스프레드 시트에 current_time / id_locate 업로드
                print(current_time, id_locate)
